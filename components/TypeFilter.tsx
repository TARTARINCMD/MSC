...
export default function TypeFilter({ selectedType, onTypeChange }: TypeFilterProps) {
  ...
  const types: (FindType | "all")[] = ["all", "track", "album", "playlist", "podcast"];
        ...
        <div className="absolute top-full left-0 mt-2 bg-card border border-border rounded-md shadow-lg z-[100] min-w-[150px]">
          {types.map((type) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm transition-colors first:rounded-t-md last:rounded-b-md capitalize ${selectedType === type
